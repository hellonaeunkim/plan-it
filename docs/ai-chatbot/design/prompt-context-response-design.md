...
5. 채팅방에 연결된 전체 대화 원문을 사용자·AI 역할 순서대로 프롬프트에 추가한다.
6. 전체 일정 데이터와 현재 질문을 구분된 블록으로 마지막 `UserMessage`에 추가한다.
...
| 구분 | 현재 상태 | 판단 |
| --- | --- | --- |
| 프롬프트 책임 | 정책은 `SystemMessage`, 일정과 현재 질문은 마지막 `UserMessage`의 구분된 블록으로 분리됨 | 경계 분리는 적용됐으며 토큰 고정 비용과 답변 영향을 측정해야 한다. |
| 일정 범위 | 사용자의 전체 일정을 조회함 | 일정이 늘어날수록 질문과 무관한 입력 토큰이 증가한다. |
| 대화 범위 | 저장된 전체 메시지를 삽입함 | 요청당 대화 토큰이 계속 증가한다. |
...
| 프롬프트 버전 | 프롬프트 생성 코드가 `ai-chat-v2`를 함께 관리함 | 메트릭으로 기준선 `ai-chat-v1`과 변경 후 결과를 구분할 수 있다. |
| 검증 기준 | 고정 평가셋과 `ai-chat-v1` 기준선이 존재함 | 같은 fixture와 평가 기준으로 이후 변경을 비교한다. |

메시지 영속화와 채팅방 소유권 검증은 선행 작업으로 완료했다. 현재 남은 핵심 문제는 전체 일정·전체 대화 전달에 따른 토큰 증가, 외부 LLM 호출의 트랜잭션 점유, 최근 3턴 적용 시 사라지는 장기 기억을 누적 요약으로 보완하는 것이다.
...
### 5.3 현재 구현 예시 (`ai-chat-v2`)

현재 구현은 시스템 정책과 사용자 데이터를 역할 수준에서 분리한다. 다만 일정 범위와 대화 원문은 아직 제한하지 않았으므로 전체 일정과 전체 이전 대화가 들어간다.
